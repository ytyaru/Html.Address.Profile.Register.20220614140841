window.addEventListener('DOMContentLoaded', async(event) => {
    ...
    function showMyData(json) {
        ...
        const field = (json.hasOwnProperty('field')) ? json.field : (primary.hasOwnProperty('field')) ? primary.field : null
        if (field) {
            for (let i=0; i<field.length; i++) {
                document.getElementById(`field-${i}-key`).value = field[i].key
                document.getElementById(`field-${i}-value`).value = field[i].value
            }
        }
    }
